...
export class KamisService {
    ...
    try {
      ...
      for (let i = 0; i < attempts; i++) {
        ...
        try {
          ...
          const response = await fetch(`/proxy.php?path=kamis/service/price/xml.do&${params.toString()}`);
   ...
   * NOW USES LOCAL NAS CACHE (generated by scripts/fetch_data.js)
   */
  static async fetchAllMajorCrops(): Promise<KamisPriceItem[]> {
    try {
      // Fetch from local static file
      const response = await fetch(`/data/kamis/daily_price.json?t=${new Date().getTime()}`);
      if (!response.ok) throw new Error('NAS Market Cache not found');
      
      const json = await response.json();
      return json.data || [];
      
    } catch (e) {
      console.warn("Using local cache failed, falling back to empty:", e);
      return [];
    }
  }
}
